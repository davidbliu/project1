...
def get_recent_messages(person, time):
	messages=Message.objects.filter(reciever=person)
	recent_messages={}
	#key message value time ago added
	for m in messages:
		time1=m.added.replace(tzinfo=None)
		time2=datetime.utcnow()
		x=minutes_between(time1,time2)
		if x<time:
			print 'there was a message added'
			recent_messages[m]=x
